...
class LicenseService
{
    ...
    public function verifyLicense ($config) {
        ...
        $this->logger->addInfo('License verification: Server responded with ['.$response.'] at ['.Carbon::now().']');
        ...
        if (property_exists($decodedResponse, 'item')) {
            ...
            $this->logger->addInfo('License verification: Product is successfully licensed at ['.Carbon::now().']');
        } else {
            $this->logger->addInfo('License verification: unlicensed/counterfeited software aborted with code 402 at ['.Carbon::now().']');
    ...
    public function verifyLicenseCode () {
        ...
        if (Storage::disk('storage')->exists($licenseFileName)) {
            ...
            if ($licenseCode === $storedLicense) {
                ...
                $this->logger->addInfo('An invalid license code ['.$licenseCodeRaw.'] was provided. ['.Carbon::now().']');
            }
        }
